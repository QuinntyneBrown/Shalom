 ...
 * and expose it as `--sh-chrome-bottom`. The CSS-only attempt at the
 ...
 * not just the bottom, so the bottom-nav floats far too high. Ported
 * from Saturdaze — see its ADR-005 and BUG-049 for the path that led
 * here.
 *
...
function trackBottomChrome(): void {
  ...
  if (!vv) {
    ...
    root.style.setProperty('--sh-chrome-bottom', '0px');
    return;
  }
  const update = (): void => {
    ...
    root.style.setProperty('--sh-chrome-bottom', `${bottomChrome}px`);
  };
