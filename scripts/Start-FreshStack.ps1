...
param(
    ...
    [string]$ConnectionString = "Server=(localdb)\MSSQLLocalDB;Database=Shalom;Trusted_Connection=True;TrustServerCertificate=True",
    [string]$SeedDir,
    [switch]$OpenBrowser
)
...
$cliProject = Resolve-FirstExistingPath `
    -Description "Shalom CLI project" `
    -Candidates @((Join-Path $backendRoot "src\Shalom.Cli\Shalom.Cli.csproj"))

$apiProject = Resolve-FirstExistingPath `
    -Description "Shalom API project" `
    -Candidates @((Join-Path $backendRoot "src\Shalom.Api\Shalom.Api.csproj"))
...
$toolPath = Join-Path $runRoot "tools\shalom"
...
# The e2e suite arrives in a later milestone; until then serve the built frontend
# with http-server resolved from e2e (preferred, mirrors saturdaze) or frontend.
if ((Test-Path -LiteralPath (Join-Path $e2eRoot "package.json")) -and
    -not (Test-Path -LiteralPath (Join-Path $e2eRoot "node_modules\http-server"))) {
    Invoke-Step `
...
Invoke-Step `
    -Title "Packing Shalom CLI" `
    -FilePath $dotnet `
    -Arguments @("pack", $cliProject, "-c", $Configuration, "-o", $packageSource, "/p:PackageVersion=$toolVersion") `
    -WorkingDirectory $repoRoot

Invoke-Step `
    -Title "Installing freshly packed Shalom CLI tool" `
    -FilePath $dotnet `
    -Arguments @("tool", "install", "Shalom.Cli", "--tool-path", $toolPath, "--add-source", $packageSource, "--version", $toolVersion) `
    -WorkingDirectory $repoRoot

$toolAssembly = Get-ChildItem -LiteralPath (Join-Path $toolPath ".store") -Recurse -Filter "shalom.dll" |
    Where-Object { $_.FullName -like "*$toolVersion*" } |
    Select-Object -First 1
if ($null -eq $toolAssembly) {
    throw "Could not find installed Shalom CLI assembly under $toolPath."
}

$resetArguments = @($toolAssembly.FullName, "--connection", $ConnectionString)
if (Test-Path -LiteralPath $SeedDir) {
    $SeedDir = (Resolve-Path -LiteralPath $SeedDir).Path
    $resetArguments += @("--seed-dir", $SeedDir)
}
$resetArguments += @("reset", "--yes")

Invoke-Step `
    -Title "Resetting database through freshly installed Shalom CLI" `
    -FilePath $dotnet `
...
Invoke-Step `
    ...
    -FilePath $npm `
    -Arguments @("run", "build", "--", "shalom", "--configuration", "development") `
    -WorkingDirectory $frontendRoot

$frontendDist = Join-Path $frontendRoot "dist\shalom\browser"
...
$httpServer = Resolve-FirstExistingPath `
    -Description "http-server binary (run npm ci in e2e once it exists, or npm install http-server in frontend)" `
    -Candidates @(
        (Join-Path $e2eRoot "node_modules\http-server\bin\http-server"),
        (Join-Path $e2eRoot "node_modules\http-server\bin\http-server.js"),
        (Join-Path $frontendRoot "node_modules\http-server\bin\http-server"),
        (Join-Path $frontendRoot "node_modules\http-server\bin\http-server.js")
    )
...
try {
    $apiProcess = Start-LoggedProcess `
        ...
        -Environment @{
            ...
            "SHALOM_CONNECTION" = $ConnectionString
        }
